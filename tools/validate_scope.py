...
def afg_setup(rec: WaveRecorder, *, prefix: str, freq: float, vpp: float,
              offset: float, wave: str = "SIN") -> list[tuple[str, str]]:
...
def main() -> int:
    ...
    p.add_argument("--average", type=int, default=10,
    ...
    with WaveRecorder(host=args.host) as rec:
        ...
        def average_check():
            ...
            ok = (avg.dtype == np.float32 and avg.shape == (F // k, mdep)
                  and np.allclose(avg, expect, atol=0.5))
        ...
        def crop_average_check():
            ...
            ok = got.shape == expect.shape and np.allclose(got, expect, atol=0.5)
        ...
        if len(enabled) > 1:
            ...
            def multi_average_check():
                ...
                bad = [c for c in enabled if not np.allclose(
                    multi[c], rec.read(count=F, channel=c, average=k), atol=0.5)]
        ...
        if use_afg:
            ...
            freq = {c: measured_hz(all_ch[c], args.sample_rate) for c in enabled}
            print("    per-channel  "
